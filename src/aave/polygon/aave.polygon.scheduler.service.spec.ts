...
import { AavePolygonSchedulerService } from './aave.polygon.scheduler.service';

describe('AavePolygonSchedulerService', () => {
  const testModule = new TestModule();
  let app: INestApplication;
  let service: AavePolygonSchedulerService;

  beforeAll(async () => {
    app = await testModule.createTestModule();
    service = await app.get<AavePolygonSchedulerService>(
      AavePolygonSchedulerService,
    );
  });

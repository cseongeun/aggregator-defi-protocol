...
import { MdexBinanceSmartChainSchedulerService } from './mdex.binance-smart-chain.scheduler.service';

describe('MdexBinanceSmartChainSchedulerService', () => {
  const testModule = new TestModule();
  let app: INestApplication;
  let service: MdexBinanceSmartChainSchedulerService;

  beforeAll(async () => {
    app = await testModule.createTestModule();
    service = await app.get<MdexBinanceSmartChainSchedulerService>(
      MdexBinanceSmartChainSchedulerService,
    );
  });
